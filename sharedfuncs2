...
### COMMON FUNCTIONS
  ...
  check_trim() {
    [[ -n $(hdparm -I ${1} | grep TRIM &> /dev/null) ]] && TRIM=1
  }
  check_root() {
    if [[ "$(id -u)" != "0" ]]; then
  ...
  check_multilib() {
    ...
    if [[ $ARCHI == x86_64 ]] && [[ MULTILIB == 1 ]]; then
  ...
  settimezone() {
    print_info "Setting timezone EU/SE"
    arch_chroot "ln -s /usr/share/zoneinfo/Europe/Stockholm /etc/localtime"
